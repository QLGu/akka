...
private[akka] object MultiStreamOutputProcessor {
  ...
  case class SubstreamSubscriptionTimeout(substream: SubstreamKey, timeout: FiniteDuration)
...
private[akka] trait MultiStreamOutputProcessorLike extends Pump with StreamSubscriptionTimeoutSupport {
  ...
  protected def createSubstreamOutput(): SubstreamOutput = {
    ...
    val cancellable = scheduleSubscriptionTimeout(self, (timeout ⇒ SubstreamSubscriptionTimeout(id, timeout)))
  ...
  val outputSubstreamManagement: Receive = {
    ...
    case SubstreamSubscriptionTimeout(key, timeout) ⇒ handlSubscriptionTimeoutManagement(substreamOutputs(key), timeout)
  }

  override protected def subscriptionTimeoutForTarget(target: AnyRef, cause: Exception) = target match {
